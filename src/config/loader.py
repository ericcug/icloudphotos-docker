...
_SCHEMA_TYPE_HINTS: Dict[str, Any] = {
    ...
    "reauth_notification_interval": 86400,
    "xmp_sidecar": False,
...
def _build_config(raw: Dict[str, Any], password: Optional[str], config_path: Path) -> Config:
    ...
    return Config(
        ...
        download_resolution=str(raw.get("download_resolution", "unmodified")),
        delete_policy=str(raw.get("delete_policy", "keep")),
        trash_days=int(raw.get("trash_days", 30)),
        delete_after_download=bool(raw.get("delete_after_download", False)),
        max_deletions_per_run=int(raw.get("max_deletions_per_run", 100)),
        icloud_china=bool(raw.get("icloud_china", False)),
        auth_china=bool(raw.get("auth_china", False)),
        log_level=str(raw.get("log_level", "info")),
        notification_days=int(raw.get("notification_days", 7)),
        wait_for_reauthentication=bool(raw.get("wait_for_reauthentication", True)),
        reauth_notification_interval=int(raw["reauth_notification_interval"]) if raw.get("reauth_notification_interval") is not None else None,
        xmp_sidecar=bool(raw.get("xmp_sidecar", False)),
        keep_icloud_recent_days=int(raw["keep_icloud_recent_days"]) if raw.get("keep_icloud_recent_days") is not None else None,
        keep_icloud_recent_only=bool(raw.get("keep_icloud_recent_only", False)),
        min_free_disk_bytes=int(raw.get("min_free_disk_bytes", 1073741824)),
